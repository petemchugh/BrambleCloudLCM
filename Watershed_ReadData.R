...
Hatch_Fish_Inputs = read.csv(Watershed.Input.File, header=F,
   skip=147, nrows=1)[, 2:8] #Pete Apr 2016 fix -- formerly referenced 2:4
    # i = 6 to 8 are not fish numbers, they're a rule ID, a cap, and a proportion
    # associated with supplementation strategies

#dim(Hatch_Fish_Inputs)
for (t in T.lo:T.hi) {
   Hatch_Fish.mu[k, 1:2, t]=0
#   Hatch_Fish.mu[k, 6:I, t] = 0
# Pete April version to deal with supplementation rules
Hatch_Fish.mu[k, 6:14, t] = 0
Hatch_Fish.mu[k, 15, t] = Hatch_Fish_Inputs[1, 5] # supplementation rule
Hatch_Fish.mu[k, 16, t] = Hatch_Fish_Inputs[1, 6] # ceiling on adults to add
Hatch_Fish.mu[k, 17, t] = Hatch_Fish_Inputs[1, 7] # proportion to subpop if < ceiling

 for (i in 3:6) { # i = 6 to 8 are not actual hatchery fish intros
